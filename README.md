...
Steps to run script and tests:

 Preconditions:
  
    1)Open projects folder. 
    2)Open terminal in this folder : 
    3)Create virtual env by using command in terminal: virtualenv -p python3 venv.
    4)Activate python vevn in terminal by command : source venv/bin/activate
    5)Install dependencies, run this command : pip3 install -r requirements.txt
    
 Run filereader script:
    
    1)Go 'main_script' folder from project folder via : cd main_script/
    2)Open terminal and run command : 'python outputlines.py'
    3)Go throw script instructions
...
Don`t hesistate to contact me: konstantin.dobro@gmail.com
